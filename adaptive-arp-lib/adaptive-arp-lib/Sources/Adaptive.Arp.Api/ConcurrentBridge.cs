          ...
          public new APIResponse Invoke(APIRequest request)
          {
               ...
               switch (request.GetMethodName())
               {
                    default:
                         // 404 - response null.
                         responseCode = 404;
                         responseMessage = "ConcurrentBridge does not provide the function '"+request.GetMethodName()+"' Please check your client-side API version; should be API version >= v2.1.8.";
                         break;
               }
