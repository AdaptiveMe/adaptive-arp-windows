...
namespace Adaptive.Arp.Impl.WinPhoneSilverlight
{
    ...
    {
        /// <summary>
        /// Shows the native media player for the specified URL
        /// </summary>
        /// <param name="url">URL of the resource to play</param>
        public void PlayStream(string url)
        {
            if (!String.IsNullOrWhiteSpace(url) && Uri.IsWellFormedUriString(url, UriKind.RelativeOrAbsolute))
            {
                MediaPlayerLauncher player = new MediaPlayerLauncher();
                player.Media = new Uri(url, UriKind.RelativeOrAbsolute);
                player.Show();
            }
        }
    }
