...
namespace Adaptive.Arp.Impl.WinPhone.Appverse
{
    ...
    public class AppverseBridge
    {
        ...
        public AppverseBridge()
        {
            ...
            appverseFunctions.Add(new Regex("^/service/phone/Call"), AppverseCall);
        ...
        private async Task<AppServerRequestResponse> AppverseCall(Stream responseStream, AppServerRequestResponse response, AppServerRequestResponse request)
        {
            ...
            Debug.WriteLine("Call: {0}", jsonRequestString);
            var requestParams = new { param1 = "", param2 = 0 };
            ...
            {
                PhoneCallManager.ShowPhoneCallUI(requestObject.param1, String.Empty);
            });          
            dataWriter.WriteString("{\"RetCallType\":0,\"RetCallStatus\":0,\"IsHangUp\":true,\"IsPause\":true,\"IsResume\":true,\"IsJoin\":true}");
        ...
        private async Task<AppServerRequestResponse> AppverseInvokeService(Stream responseStream, AppServerRequestResponse response, AppServerRequestResponse request)
        {
            ...
            if (ioService != null)
            {
                ...
                else if (ioService.Endpoint.Path.EndsWith("/rest/sec/logout"))
                {
                    string jsonResultString = "{\"ContentBinary\":null,\"GetContentLengthBinary\":0,\"ContentType\":\"application/json\",\"Headers\":[{\"Name\":\"Cache-Control\",\"Value\":\"max-age=0\"}],\"Content\":\"\",\"Session\":{\"Cookies\":[{\"Name\":\"JSESSIONID\",\"Value\":\"BF0JzFWxvKDLugj5S+M9DcCm.sit-p2pMobileServer1\"},{\"Name\":\"JSESSIONID\",\"Value\":\"BF0JzFWxvKDLugj5S+M9DcCm.sit-p2pMobileServer1\"},null,null,null,null,null,null,null,null,null]}}";
                    string jsonContent = "{\"result\":{\"success\":true,\"code\":\"0\",\"message\":\"Esito positivo\",\"warning\":false,\"guid\":null}}";
                    JsonSerializerSettings settings = new JsonSerializerSettings();
                    settings.StringEscapeHandling = StringEscapeHandling.EscapeHtml;
                    IOResponse jsonResultObject = JsonConvert.DeserializeObject<IOResponse>(jsonResultString);
                    jsonResultObject.Content = jsonContent;
