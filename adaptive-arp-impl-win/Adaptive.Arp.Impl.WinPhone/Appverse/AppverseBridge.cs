...
namespace Adaptive.Arp.Impl.WinPhone.Appverse
{
    ...
    public class AppverseBridge
    {
        ...
        private async Task<AppServerRequestResponse> AppverseGetResourceLiterals(Stream responseStream, AppServerRequestResponse response, AppServerRequestResponse request)
        {
            ...
            foreach (String loc in globalization.GetLocaleSupportedDescriptors())
            {
                Debug.WriteLine("Locale: {0}", loc);
            }
            IDictionary<string, string> literals = globalization.GetResourceLiterals(new Locale(requestObject.param1,requestObject.param1.ToUpper()));

            String jsonDict = JsonConvert.SerializeObject(literals);
            Debug.WriteLine("Literals Dict: {0}", literals);
            Debug.WriteLine("Literals JSON: {0}", jsonDict);
            */
