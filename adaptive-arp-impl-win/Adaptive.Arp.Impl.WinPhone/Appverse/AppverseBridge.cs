            ...
            if (ioService != null)
            {
                if (ioService.Endpoint.Path.EndsWith("/rest/sec/login"))
                {
                    ...
                    Task.Run(async () =>
                    {
                        ...
                        {
                            Debug.WriteLine("Invoking Listener: {0} Id: {1} Eval: eval({2});", callbackFunction, callbackId, jsCallbackFunction);
                            (AppContextWebviewImpl.Instance.GetWebviewPrimary() as WebView).InvokeScriptAsync("eval", new string[1] { "console.log('\"" + jsCallbackFunction + "\"')" });
