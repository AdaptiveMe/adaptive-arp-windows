...
//			Encoding e = Encoding.GetEncoding (name, EncoderFallback.ExceptionFallback, DecoderFallback.ExceptionFallback);
		...
		{
            CultureInfo r = CultureInfo.CurrentCulture;
			return r;
		}
		...
		{
			try {
                TimeZoneInfo tz = null;// TimeZoneInfo.FindSystemTimeZoneById(tzone);
				return tz;
			} catch {
				// Not found
			}
			...
			if (tzone[3] == '-')
				t = -t;
            return null;// TimeZoneInfo.CreateCustomTimeZone(tzone, t, tzone, tzone);
		}
		...
		public static string GetTestName ()
		{
			...
			int n = 0;
			do {
                met = null;// new StackFrame(n).GetMethod();
				if (met != null) {
