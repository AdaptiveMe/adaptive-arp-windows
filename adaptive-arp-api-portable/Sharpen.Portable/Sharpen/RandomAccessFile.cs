	...
	internal class RandomAccessFile
	{
		...
		public RandomAccessFile (string file, string mode)
		{
			if (mode.IndexOf ('w') != -1)
                ...
                OpenStream(file, FileAccess.ReadAndWrite);
			else
				//stream = new FileStream (file, System.IO.FileMode.Open, FileAccess.Read);
                OpenStream(file, FileAccess.Read);
		}

        private async Task OpenStream(string fileName, FileAccess mode)
        {
            IFile fileTask = await FileSystem.Current.LocalStorage.GetFileAsync(fileName);
            if (fileTask != null)
            {
                stream = await fileTask.OpenAsync(mode);
            }
            else
            {
                stream = null;
