...
namespace System {
	internal static class UriHelper {
		...
		private static UriSchemes GetScheme (string schemeName)
		{
			...
			if (schemeName == UriPort.UriSchemeHttp)
				return UriSchemes.Http;
			if (schemeName == UriPort.UriSchemeHttps)
				return UriSchemes.Https;
			if (schemeName == UriPort.UriSchemeFile)
				return UriSchemes.File;
			if (schemeName == UriPort.UriSchemeFtp)
				return UriSchemes.Ftp;
			if (schemeName == UriPort.UriSchemeGopher)
				return UriSchemes.Gopher;
			if (schemeName == UriPort.UriSchemeLdap)
				return UriSchemes.Ldap;
			if (schemeName == UriPort.UriSchemeMailto)
				return UriSchemes.Mailto;
			if (schemeName == UriPort.UriSchemeNetPipe)
				return UriSchemes.NetPipe;
			if (schemeName == UriPort.UriSchemeNetTcp)
				return UriSchemes.NetTcp;
			if (schemeName == UriPort.UriSchemeNews)
				return UriSchemes.News;
			if (schemeName == UriPort.UriSchemeNntp)
				return UriSchemes.Nntp;
			if (schemeName == UriPort.UriSchemeTelnet)
				return UriSchemes.Telnet;
			if (schemeName == UriPort.UriSchemeUuid)
		...
		internal static bool HasCharactersToNormalize (string str)
		{
			...
			for (int i = 0; i < len; i++) {
				...
				int iStart = i;
				char surrogate;
				char x = UriPort.HexUnescapeMultiByte (str, ref i, out surrogate);
			...
			UriComponents component, UriFormat uriFormat, FormatFlags formatFlags)
		{
			...
			for (int i = 0; i < len; i++) {
				...
				if (c == '%') {
					...
					char x = UriPort.HexUnescapeMultiByte (str, ref i, out surrogate, out invalidUnescape);
